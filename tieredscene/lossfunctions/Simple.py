...
class SimpleDataLossFunction(DataLossFunction.DataLossFunction):
    ...
    def __call__(self, pixel, label):
        return abs(pixel.value - self.dict[label]) /10.0
        #raise NotImplementedError("TODO: fill this in using the per pixel class confidences from reference 9 from the paper")
    
    

class SimpleSmoothnessLossFunction(SmoothnessLossFunction.SmoothnessLossFunction):
    _label_set = gcl_label_set
    _hloss_table = numpy.array([[0, 1, 4, 1, 1 ],
                                [1, 0, 1, 1, 1 ],
                                [10, 1, 0, 1, 1 ],
                                [1, 1, 1, 0, 1 ],
                                [1, 1, 1, 1, 0 ]
...
DataLossFunc  = SimpleDataLossFunction
SmoothnessLossFunc = SimpleSmoothnessLossFunction
